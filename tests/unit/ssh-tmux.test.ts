...
describe('buildTmuxLaunchCommand forces session-scoped mouse off (#546)', () => {
  ...
  // its status-interval repaints don't reset the watchdog's silence clock (the
  // sleep indicator must track claude, not tmux heartbeats). Session-scoped like
  // the mouse-off beside it.
  it('also forces the tmux status bar off, session-scoped, on both branches', () => {
    ...
    // Present on the attach branch AND in the fresh pane (which appears twice —
    // the attach fallback and the else branch), so three occurrences total,
    // matching the mouse-off it rides beside.
