...
/**
 * Codex MCP TOML injection -- conductor-vision config.toml support.
 *
 ...
 * MCP endpoint Claude consumes via `~/.claude/settings.json` is exposed to
 * Codex via this file, with no server-side changes -- Codex speaks the
 * same MCP protocol.
 *
 * Spec: docs/superpowers/specs/2026-04-29-codex-integration-design.md sec 12.2.
 */

const MARKER_SECTION = '[mcp_servers.conductor-vision]'
...
/**
 * Strip our managed conductor-vision block from a TOML string. Walks
 * line-by-line so user-authored sections surrounding the block are left
 * untouched. Returns the original string if the block is absent.
 *
...
function stripManagedBlock(content: string): string {
  if (!content.includes(MARKER_SECTION)) return content

  const lines = content.split('\n')
  const sectionIdx = lines.findIndex((l) => l.trim() === MARKER_SECTION)
...
/**
 * Write a `[mcp_servers.conductor-vision]` HTTP/SSE entry into the user's
 * Codex config.toml so Codex sessions reach the same MCP server Claude does.
 *
 ...
 * already exists (e.g. from a prior run on a different port), it is
 * stripped first so the new port takes effect. This mirrors Claude's
 * `injectMcpSettings` JSON overwrite semantics; without it a port change
 * in vision config would leave a stale URL behind.
 */
export function injectConductorVisionInCodexConfig(port: number): void {
  const codexHome = getCodexHome()
  if (!existsSync(codexHome)) {
    logInfo('[codex-mcp] ~/.codex not present; skipping conductor-vision injection')
    return
  }
  ...
  try {
    ...
    logInfo(`[codex-mcp] injected conductor-vision into ${tomlPath} (port ${port})`)
  } catch (err: any) {
...
/**
 * Remove the conductor-vision block (and our managed-by comment) from the
 * Codex config.toml. No-op when the file or block is absent.
 */
export function removeConductorVisionFromCodexConfig(): void {
  ...
  if (!content.includes(MARKER_SECTION)) return

  const stripped = stripManagedBlock(content)
  try {
    writeFileSync(tomlPath, stripped, 'utf-8')
    logInfo(`[codex-mcp] removed conductor-vision from ${tomlPath}`)
  } catch (err: any) {
    logError(`[codex-mcp] failed to write ${tomlPath}: ${err?.message}`)
  }
}
