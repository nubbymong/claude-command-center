...
export function saveSessionState(state: SessionState): boolean {
  try {
    ...
    console.log(`[session-state] Saved ${state.sessions.length} sessions`)
    return true
  } catch (err) {
...
export function loadSessionState(): SessionState | null {
  try {
    ...
    console.log(`[session-state] Loaded ${state.sessions.length} sessions from ${new Date(state.savedAt).toLocaleString()}`)
    return state
  } catch (err) {
...
export function clearSessionState(): boolean {
  try {
    ...
    if (existsSync(file)) {
      unlinkSync(file)
      console.log('[session-state] Cleared saved state')
    }
    return true
  } catch (err) {
