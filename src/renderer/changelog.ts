...
export const changelog: ChangelogEntry[] = [
  {
    ...
    changes: [
      ...
      { type: 'feature', description: 'Ctrl+wheel zooms the canvas pane like a browser — over the chrome, the render or the notes panel. The level shows in the header and sticks for the session.' },
      { type: 'improvement', description: 'The Saved Configs panel grew into a real launcher: cards or a compact list (chosen in Settings), find-as-you-type with auto-complete, categories, running sessions excluded from the launch list, and a launch-all for a category.' },
      ...
      { type: 'fix', description: 'Windows: tools built for the GUI subsystem (the Slic3r family, Inkscape, some Qt and Git components) could attach to the app\'s console and bleed their log text over your terminal. They are spawned without a console now, so their output goes nowhere near your session.' },
      ...
      { type: 'improvement', description: 'Ask Conductor can reach Sentinel now, and has its own Feature Guide entry, so "why was that flagged" can be asked where the answer is.' },
      { type: 'improvement', description: 'Session state survives more failures: saving is atomic, a restore that cannot read the file fails open instead of discarding your sessions, and what could not be read is preserved rather than overwritten.' },
      { type: 'improvement', description: 'The Session Watchdog reads the real screen now. It watches a rendered pane — the same thing you see — instead of an append-only log, so an in-place redraw cannot pin it on stale text. Before typing a retry it checks the pane: nothing is sent while a permission prompt or picker is open or your own unsubmitted draft is in the input box, and the dim placeholder hints Claude shows in an empty input no longer count as a draft, so the retry actually fires when the box is empty. The tuning knobs (backoff, margins, retry caps) are reachable from settings now, within hard bounds — the patterns that decide when it may type stay compiled into the app.' },
      { type: 'improvement', description: 'GPU terminal rendering is ON by default now that the shared-cache corruption is actually fixed. If you ever see characters go missing while backgrounds stay, Ctrl+Alt+G saves a diagnostic (event log plus screenshot) worth attaching to a report, and Settings > General > GPU rendering falls back to the plain renderer.' },
