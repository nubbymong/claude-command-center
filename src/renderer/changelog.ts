...
export const changelog: ChangelogEntry[] = [
  {
    ...
    changes: [
      ...
      { type: 'fix', description: 'On Linux, the check for the Claude CLI no longer assumes zsh when no login shell is set in the environment. It falls back to bash, or to /bin/sh where bash is absent, so a machine without zsh is not told the CLI is missing when it is installed.' },
    ],
  },
  {
    version: '2.1.0',
