...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: '2026-09-25',
    highlights: 'Codex becomes a full second assistant beside Claude Code. One Accounts page covers both: turn each on or off, see whether it is installed and up to date, and keep more than one Codex account, each with its own sign-in. A saved config picks the Codex account it runs under, a Codex session can restart into an earlier conversation, and each assistant can ask the other to review its work. A new install asks which assistants you use, and the app can run with Codex alone.',
    changes: [
      { type: 'feature', description: 'Settings, Accounts is now one page for both assistants. A Providers card at the top turns Claude Code and Codex on or off, says whether each one is installed and which version, and, when Codex is missing or too old, lists the install or update commands from OpenAI\'s own instructions to copy, with Check again. Your Claude accounts and your Codex accounts are listed below it. At least one provider always stays on, and a provider cannot be switched off while anything of it is still running.' },
      { type: 'feature', description: 'More than one Codex account. Add one with ChatGPT, a device code or an API key; the key goes to Codex, and this app never stores it. Each Codex account signs in inside its own folder, which the app creates in its resources folder, so sessions never mix identities, and the app never reads the sign-in Codex keeps there. Each account row has a menu: Make default, Make reviewer, Sign in again, Check sign-in, Sign out, Make inactive and Archive.' },
      { type: 'feature', description: 'The Codex sign-in already on your computer can be used too. Once you say you use Codex, the app asks Codex whether it is signed in and offers it in Settings, Accounts. Because the app did not create it, you confirm it at each launch, and it never runs code reviews; if it is signed out, run codex login in a terminal, then Check sign-in.' },
      { type: 'feature', description: 'New saved config picks the Codex account a config runs under, with the default listed first; an account that needs attention cannot be picked. A Codex session header has a Restart menu: Restart starts a new conversation, and Restart and pick a conversation lists recent ones in the terminal to pick from.' },
      { type: 'feature', description: 'Code review works both ways. A Claude session can ask for a Codex review, and a Codex session can ask for a Claude review. Each review is a separate, one-off, read-only reviewer in the project, on your reviewer account: the reviewer default, or the default account when none is set. Settings, General, Built-in Tools has a switch for each direction that names the account reviews will use and, when a review cannot run, says why. A review tool is now offered only while a review could actually run. On macOS, Claude reviews use your normal Claude sign-in.' },
      { type: 'feature', description: 'A fresh install asks which assistants you will use: Claude Code, Codex, or both. A computer without Claude Code can choose Use Codex only on the setup screen instead of stopping. Choosing Codex leads to a Set up Codex page that checks for the Codex CLI, can run the npm or Homebrew install or update command in a visible terminal tab once you confirm it, and signs you in.' },
      { type: 'feature', description: 'Hello Codex: a five-page introduction to Codex in this app, shown once when you have said you use Codex and a Codex account you added is signed in. Replay it from Show the Codex introduction on the Codex card in the Feature Guide, or on the Codex row in Settings, Accounts.' },
      { type: 'improvement', description: 'A provider that is off starts nowhere, whichever way a session is started: a saved config says why instead of launching, and a restored or restarted tab reads Not started, names the provider and points at Settings, Accounts. The tab and its conversation are kept, so turning the provider back on and restarting the tab carries on. With Claude Code off, Ask Conductor, Cloud Agents and Insights say so, and terminal-only configs still run.' },
      { type: 'improvement', description: 'Signing a Codex account in again asks you to confirm it is the same account as before, and offers the same kind of sign-in it had. When a check finds a Codex account now signed in a different way than before, such as an API key where it had a ChatGPT sign-in, the account reads Needs attention and waits for you to confirm it with This is still my account before any session or review runs on it.' },
      { type: 'improvement', description: 'The old Settings Codex tab and its single Codex sign-in are gone: everything they did now lives in Settings, Accounts, and Test connection is Check sign-in on each account. The ChatGPT plan label is no longer shown, because it came from a Codex sign-in file this app no longer reads. If a Codex config says Sign in to Codex first after the upgrade, open the Codex section of Settings, Accounts and press Yes, I use Codex (or Use this computer\'s Codex sign-in, or Check again) to use the Codex sign-in you already had. Codex sessions and Codex reviews run on this computer only in this release, and the session dialog says why its SSH options are off for Codex.' },
      { type: 'improvement', description: 'Keyboard focus is easier to see: switches, and every field and list in Settings, show a clear ring when you reach them with Tab. Status labels on the Conductor MCP page and the badges in setup are easier to read, in both themes.' },
      { type: 'fix', description: 'SSH sessions with tmux now start Claude on a host whose login shell is zsh, the macOS default: zsh read the remote session name as a command and stopped the start line. For the same reason, End did not close that remote session or remove its files; now it does.' },
      { type: 'fix', description: 'Ending a session in a container that needs sudo, when the session started without the sudo password saved in the config (it was typed at the prompt), no longer reports success while Claude keeps running inside the container. End still ends the remote session and removes its files on the host, then says Claude may still be running in that container and shows a command, to run on the host, that stops Claude and removes its files in the container. To let End do it itself, save the sudo password in the config (Edit, then in Runtime enter the Sudo password with Save password left ticked); that applies to sessions started after you save it.' },
      { type: 'fix', description: 'End now also stops Claude in a container that has no bash (a minimal image), and closing several sessions at once (the ones selected with Ctrl-click, or Cmd-click on a Mac, or Close all sessions in a group or section) ends a container session the way closing its tab does. Before, both left Claude running inside the container.' },
      { type: 'fix', description: 'On Windows, an SSH session whose connection was refused straight away could close the whole app. The session now just ends, and the rest of the app keeps running.' },
    ],
  },
  {
    version: '2.1.1-beta.1',
    date: '2026-09-16',
    highlights: 'The mouse wheel scrolls a remote session the way it scrolls a local one, instead of typing arrow keys at Claude. Otherwise a maintenance release: the runtime and its dependencies refreshed, and the largest source files split into focused modules with no change in behaviour.',
    changes: [
      { type: 'fix', description: 'The mouse wheel scrolls the history of an SSH Persistent session, the same as a local one. It used to send arrow-key presses to Claude instead -- so a remote session could not be scrolled back at all, and every notch typed an arrow key at whatever was running, which in a full-screen program could move the cursor or change what was selected. Claude runs inside tmux on an SSH Persistent session and the terminal has no scrollback of its own there, so the wheel now drives tmux’s own scrollback instead. Selecting text with the mouse is unchanged. Scrolling up opens the scrollback view and the first key you type closes it and returns you to the live bottom, so you can scroll back to read something and just start replying.' },
  ...
  },
  {
    ...
    changes: [
      { type: 'feature', description: 'The app is now AI Code Conductor, with a new name, icon, start-up animation and session setup dialog. Your existing installation upgrades in place and keeps every config, account and saved session -- nothing needs re-creating. The first launch after upgrading explains what changed and previews what 2.2 is aiming at: support for more coding agents beside Claude Code and Codex.' },
      { type: 'feature', description: 'A remote session is now a full session. SSH sessions run inside tmux on the remote, so closing the lid, losing wifi or switching networks no longer kills the work -- reconnecting picks it back up where it was. Local and remote sessions report their status over the same channel, so an SSH session shows its account and its full usage, per-model bars included, exactly like one on your own machine. That includes Windows remotes, which never reported at all before. A remote you left running waits in a Remote Resumable section at the foot of the Running tab, one click from the same Claude and the same conversation.' },
      { type: 'feature', description: 'The session config dialog answers "where does this run?" properly. Connection is a choice of three -- Local, SSH, SSH Persistent -- and a new Runtime section runs the session inside a Docker or Podman container without you maintaining a shell one-liner. Entering a container is confirmed from inside the container before anything is launched there, and checked again before Claude itself starts, so a container that stopped in between gets nothing typed onto the host.' },
      { type: 'feature', description: 'The Agent Canvas is a place your agent shows you work and you mark it up. It handles plans, mockups and the built site, anchored notes go on the thing you are pointing at, and a review is a decision that settles: approving a version owes the agent nothing, and an approved artefact signs itself off. A Testing mode lets you click through the build your agent is serving and save evidence notes that lock the screen, the page state and how you got there together into a pass-or-fail pack. Unfinished canvas work outlives the session that made it -- any session on the project can pick it up -- and a Canvas Explained page draws the whole model.' },
      { type: 'feature', description: 'claude.ai comes into the app. Each account holds its own web session in a dedicated per-account browser, kept fully separate, and the whole sign-in can be routed into the in-app pane instead of a separate window -- which is also what gets past Cloudflare’s "verify you are human" loop. The browser pane has a pinned "claude.ai -- your artifacts" entry that opens signed in as the session’s own account.' },
      { type: 'fix', description: 'The terminal corruption is understood and gone. GPU rendering shared one cache of character images across every open session, so one session rebuilding that cache wiped the text out of all the others -- which is why it looked random, why it got worse the more sessions you had, and why resizing the window brought it back. That cache is a limit of the drawing library and is still shared, so the repair is elsewhere: only the terminal you are looking at draws on the GPU at all, and a terminal about to lose its characters now drops its own drawing model first and then repaints -- which is exactly what resizing the window did by hand. GPU rendering is on by default as a result, and stale glyphs left after scrolling are cleared.' },
      { type: 'feature', description: 'A Session Watchdog waits out a rate limit and types the retry itself. Its three auto-retry checks -- rate-limit resume, API overload and safeguard -- are separate switches in Settings, and each session can turn them on or off for itself from its right-click menu, taking effect immediately. A Watchdog pill in the session header says what is live for that session. It is careful never to type over you, and the sleeping-session indicator only ever reports and never types.' },
      { type: 'feature', description: 'Session cards show life at a glance. A session actively producing output gets a green "working" pill and a sweep across its context bar; one that has gone quiet gets a moon with how long it has been asleep. Clicking a sleeping session does not wake its moon -- only real output does.' },
      { type: 'feature', description: 'The left panel has two modes, Saved configs and Running sessions, with a Quick Start. The command bar under each session is one row: the fixed tools, then your Global buttons, then this config’s Session buttons, and the bar knows what kind of session it is in -- a Codex session says Codex, an SSH session says which computer each button runs on. A config can be marked Allow Multi Spawn, so the ones you genuinely want several copies of get a copy-count control and a Select mode that launches a batch, and everything else is protected from a stray second launch.' },
      { type: 'feature', description: 'Every page -- Tokenomics, Usage, Settings and the new Feature Guide -- opens as a tab beside your sessions instead of taking over the window. Ask Conductor is a slim help session that answers questions about the app itself, and it can install a helper skill so every Claude session on this machine can answer them too, without opening the Ask tab.' },
      { type: 'improvement', description: 'Resuming your work is more reliable. A conversation that was running in a git worktree comes back correctly even after that worktree is cleaned up, instead of opening a fresh empty session, and the name you gave a session travels with the conversation so the resume list uses your own name for it. The Usage page loads account by account in place, so a slow account no longer holds the whole page blank, and Codex spend is counted correctly.' },
      { type: 'feature', description: 'Experimental Linux support: AI Code Conductor runs on Linux as an AppImage, alongside Windows and macOS. It needs glibc 2.39 or newer (Ubuntu 24.04+, Rocky 10+), and the vision browser needs a deb or rpm Chrome rather than a snap.' },
      { type: 'improvement', description: 'The runtime moved to Electron 43, with the terminal backend and the local database updated to match. Ctrl+V pastes into terminals, which also makes voice dictation and text expanders work, and Check for Updates can install the update it finds. Your own Claude hooks run in the app’s sessions, and each config can set its own permission mode and CLI arguments.' },
      { type: 'fix', description: 'A security line through the whole release: several local-attacker vulnerabilities were found and fixed across the local tools server, the session-launch path, the updater and the bundled dependencies, each with an advisory published alongside the release that carried it, and every open dependency alert on the project was cleared. Security-sensitive changes now go through an independent adversarial review before they can merge.' },
      { type: 'fix', description: 'A long tail of paper cuts from the release-candidate pass: startup pages take strict turns so a tour and a restore prompt no longer wait for each other and show neither; a right-click menu near the bottom of the window opens upward instead of running off the edge; on a Mac, Close sessions closes the sessions and quitting asks first; hovering a link no longer flickers the terminal; and a setup page taller than your screen scrolls instead of hiding its buttons.' },
    ],
  },
  {
    version: '2.1.0-rc.17',
  ...
  },
  {
    ...
    changes: [
      { type: 'fix', description: 'A container entry is confirmed from inside the container before anything is launched there. The app enters a container with a command that prints a one-time confirmation from inside it before the shell starts, and only that confirmation makes the session count as inside: a prompt that merely looks different, a quiet pane, a cancelled sudo prompt or a host prompt the app cannot recognise (a zsh prompt ending in a percent sign) never does. Launching re-checks the attached shell first, and again before Claude itself is started, so a container you detached from or that stopped in between gets nothing typed onto the host. A container configured to Start, or an after-connecting command that is not a plain docker or podman exec, cannot be confirmed this way: the app says so and asks before launching. Configs written as a free-text exec line get the same confirmation and keep the shell they name, and the Runtime section has a "Shell inside the container" choice for images without bash.' },
      { type: 'fix', description: 'Two sessions on one account can no longer capture a half-written sign-in. The backup that follows a token rotation was checked once per session instead of once per account, so with two sessions open a sign-in could be backed up between its two writes and restored later as a mixed account.' },
      { type: 'fix', description: 'A session started while its account\'s token is being refreshed now waits for the refresh to finish before it starts, as background runs already did, so it never reads a token that is about to be replaced. Closing the tab during the wait cancels the start; anything typed during the wait is dropped rather than queued into the shell.' },
      { type: 'fix', description: 'Capturing a second account that was signed into mid-session no longer reinstalls a token the first account may already have spent. The original account gets its identity back and shows Sign in; signing in once brings it back, instead of it failing silently later on a dead token.' },
      { type: 'fix', description: 'Cancelling an agent before it has started really cancels it. An agent cancelled while it was still waiting for an account refresh or a Claude install used to start anyway, labelled cancelled, with no control left to stop it.' },
      { type: 'fix', description: 'On a Mac, Close sessions closes the sessions. Choosing Close sessions on the close dialog discarded the cards but left the sessions running invisibly behind the Dock icon; they are ended first now, and anything left over is swept when the last window closes.' },
      { type: 'fix', description: 'Closing the window before answering the resume prompt keeps everything. With both saved sessions and left-running remotes, closing before choosing Resume or Don\'t open cleared the saved file, remotes included; the file is left as it is now, and the next launch asks again.' },
      { type: 'fix', description: 'A remote left running is never pruned because its tmux cannot answer. A tmux client that exists but cannot talk to its server (after a tmux upgrade, or a permission problem on the socket) used to read as "no sessions" and the entry was removed as dead; the check now tells that apart from a genuine empty answer and keeps the entry, marked as not verified.' },
      { type: 'fix', description: 'Output from a restarted session\'s old process no longer reaches the new one. Late output from the replaced process was still shown in the new terminal and fed to its watchdog; it is dropped now, as its exit already was.' },
      { type: 'fix', description: 'A rejected plan no longer hides from the canvas resume list behind an approved design. Resume judged a canvas by its newest artefact only; it now looks at every artefact and at every note still owed, so a canvas with a rejected plan that was never reworked, an unsubmitted note, or an answered note you have not looked at stays resumable, and Mark complete says which artefact was rejected and not yet reworked.' },
    ],
  },
  {
    version: '2.1.0-rc.15',
    ...
    changes: [
      ...
      { type: 'fix', description: 'The terminal, browser, Agent Canvas and logs pane are now one at a time. Opening the canvas while the browser was open did nothing — the browser held the foreground and the canvas never appeared. Opening any of the browser, the canvas or the logs pane now switches to it and closes the others; clicking the pane you are already in returns you to the terminal.' },
      { type: 'feature', description: 'The Usage page loads one account at a time, in place. Opening it shows a row per account straight away and fills each one in as its figure arrives, so a slow account no longer holds the whole page blank, and an account whose fetch fails says so on its own row instead of stalling the rest.' },
      { type: 'improvement', description: 'An account with an open session shows the usage its session already reported, instead of asking Anthropic again; the primary account is the exception and is always fetched. Accounts with no session open are fetched one at a time, so opening the page no longer fires a request per account (which was drawing rate-limit errors), and it never refreshes a signed-in account\'s token behind a running session. The one exception: an account on extra usage (paid credits) still makes a single request for the credits figure the status line cannot carry, with the token it already has.' },
      { type: 'fix', description: 'A token refresh can no longer sign out an account that a background run was using. The Usage page refreshes an account\'s token only while nothing is using it, but headless runs (Insights, cross-account synthesis, Sentinel), cloud agents and plain shells under an account were invisible to that check, so a refresh could rotate the token under them and the next start of that run redeemed an already-spent token and logged the account out. Every such run now counts as using the account for as long as it runs, and one that starts while a refresh is in flight waits for the new token before it reads anything.' },
      { type: 'fix', description: 'Editing a saved SSH config can no longer point a left-running session at the wrong host. A session you leave running is remembered with the host, user, port, path and container it was left on; if you then change where that config connects, the app no longer probes or ends a session of that name on the new host. The old session shows as retargeted in Remote Resumable, with Remove as its only action, and tells you how to end it on the host it is really on.' },
      { type: 'improvement', description: 'Editing a config that has a session running warns you first. Opening the editor for an SSH config with a live or left-running session shows what is affected and reminds you that changes apply on the next launch, not to the running session, and that changing where it connects can break resuming the one you left running. Cancel, or Edit anyway; it never blocks.' },
      { type: 'fix', description: 'A saved sudo password is never typed into a container. The password kept for a container entry that needs sudo answers the host\'s own sudo prompt, before the container shell appears; a sudo-shaped prompt printed inside the container afterwards, by a start-up script or by something you ran, gets nothing.' },
      { type: 'fix', description: 'Quitting no longer hangs when the app\'s window has crashed. If the window\'s renderer died while the save-or-cancel question was up, the quit waited forever for an answer that could not come; it proceeds now. Pressing Quit a second time while the question is showing also goes through, as a second close already did.' },
      { type: 'fix', description: 'A left-running remote cannot be revived by text a host prints on connect. The check that asks a host which sessions still exist reads only what the host printed between the check\'s own markers, so a login banner or a message of the day can no longer make a finished session look alive in Remote Resumable.' },
      { type: 'fix', description: 'Quitting on a Mac asks before it stops anything. Cmd+Q or Quit used to shut down every session and background service first and show the save-or-cancel dialog afterwards, so Cancel handed you back an app that had already stopped your work. The question now comes first: Cancel keeps everything running, and Save shuts down only after your sessions are written.' },
      { type: 'fix', description: 'Reopening from the Dock works on a Mac. Closing the last window and then clicking the app\'s Dock icon could bring back a blank, hidden window, or take the app down, because the window\'s controls were being registered a second time. They are registered once now.' },
      { type: 'fix', description: 'Restarting a session no longer shows it as exited. The old process\'s exit could arrive after the new one had started and mark the healthy replacement as ended, so Ask Conductor treated the session as dead and started yet another one. An exit from a process that has already been replaced is ignored.' },
      { type: 'fix', description: 'Remote sessions you left running are remembered even when they were the only thing open. Leaving the last SSH session running and then closing the app forgot its Remote Resumable entry, so the next launch of that config started new work instead of reattaching. The list of left-running remotes is now kept on its own, including when you decline the resume prompt or close without saving.' },
      { type: 'fix', description: 'Left-running remotes on a Mac with a Homebrew tmux are no longer pruned as dead. The liveness check could not see a tmux installed outside the login PATH, took its empty answer as proof, and removed live entries; it now looks in the same places the End command does.' },
      { type: 'fix', description: 'Signing in again for an expired account waits for the actual sign-in. The prompt used to report success after four seconds whether or not you had signed in, because the account\'s email was still on disk from before. It completes only once the credentials have really changed.' },
      { type: 'fix', description: 'Capturing a second account signed into mid-session no longer risks the first one. The app restores the original account from a backup that was refreshed only when the session ended, so a token that had rotated during the session could be replaced by a stale one that no longer worked. The backup now follows each rotation that happens while a session on that account is open.' },
      { type: 'fix', description: 'A session\'s transcript-indexing opt-out survives save and restore. A restored session dropped the setting and was indexed regardless.' },
      { type: 'fix', description: 'A container that cannot be entered no longer puts Claude on the host. When the container was stopped or missing, or the engine was down, the failed entry dropped back to the host shell and the app took that for the inside of the container, so Launch Claude set up and started Claude on the host instead. The failure is recognised now, from the engine\'s or sudo\'s message or from the host\'s own prompt coming back: the session says so, Launch Claude refuses the host, Run again re-enters the container once it is running, and Skip remains the explicit way onto the host shell.' },
      { type: 'fix', description: 'The saved SSH password is no longer typed into a sudo prompt. After a key-based login the saved password was still waiting for a prompt, and a sudo prompt shaped as a bare "Password:" (the macOS form) matched it first, so the wrong secret went to sudo and the first attempt failed even with a sudo password saved. Once the login has completed, such a prompt is treated as sudo\'s, whether or not the session runs a post-connect command.' },
      { type: 'fix', description: 'Closing a container session tab ends the Claude inside the container. Container sessions never showed the End-or-Leave dialog, and the plain close only killed the local connection, which does not reliably stop the process inside the container; abandoned Claudes accumulated there. Close now ends that session\'s process in the container first, without touching another session sharing it.' },
      { type: 'fix', description: 'The browser pane stays behind whatever you open on top of it. The pane, and the claude.ai artifacts view that can live in it, are drawn by the app itself rather than by the page, so nothing on the page can sit over them: they have to be told to step aside. They stepped aside when you switched sessions, but not when you opened a page tab, so opening Settings (or Tokenomics, Logs, the Feature Guide) with artifacts open left the artifacts view in front of the page you asked for. Every page tab now hides the pane while it is in front, and so does every window-covering dialog, the guided tour and the welcome walkthrough; the pane comes straight back when you return to the session, and a page that arrives while something is on top waits until the pane may show rather than flashing over it.' },
  ...
  },
  {
    ...
    changes: [
      { type: 'feature', description: 'Ask Conductor can install a helper skill for all your sessions. Say "install the helper skill" in the Ask tab and it copies one ready-made skill file into your Claude configuration — you approve the single write — after which any Claude session on this machine can answer Conductor questions itself: why a setting did not apply, which settings file wins, how accounts share settings. The skill reads the app\'s own regenerated documentation, so it never goes stale; say "remove the helper skill" to uninstall it, and ask about the portable copy if you want a remote machine to have it too. It acts only when you ask.' },
      { type: 'feature', description: 'Ask Conductor has a page of its own in the setup tour, shown on first install and after updates, with the new conductor mark beside the heading — the same mark that now sits on the Ask Conductor button at the foot of the sidebar. The What\'s New summary line grew its "See it" chip to jump there.' },
      { type: 'fix', description: 'Hovering a link no longer flickers. The terminal\'s link detector re-checks the link under your cursor every time the screen repaints — and a Claude session repaints constantly — so the hover state strobed at that pace; worse, the strobe drove the app\'s own cursor-hiding style, so in a busy session the flicker you saw was the screen itself blinking. The hand cursor is now managed outside that churn: it appears instantly, holds steady through repaints, and the right-click menu\'s Copy link address no longer randomly goes missing mid-animation.' },
      { type: 'improvement', description: 'Links in the terminal have been clickable since rc.10 — this note is the announcement that release missed. Click an https link to open it in your browser; right-click any http or https link for Copy link address. Works in local and remote sessions alike.' },
      { type: 'fix', description: 'A setup page taller than your screen now scrolls. On a smaller display, the "What you\'re getting" page (and any other tall setup step, the Welcome and Status Line pages included) could push its only buttons off the bottom edge with no scrollbar anywhere, leaving the tour stuck. Every step now scrolls within the page while the buttons stay put.' },
      { type: 'improvement', description: 'Fable 5.1 is in the model picker. The registry and the shipped model list now carry claude-fable-5-1 from Anthropic\'s Claude Code model configuration, so it appears in both pickers, and the "Fable" row always means the newest Fable.' },
      { type: 'fix', description: 'Sentinel stopped crying wolf about new models. A notice that Anthropic published a model your picker does not offer yet was displayed as "1 severe breaking change" with a red breaking badge — alarming words for something that breaks nothing you are running. Findings are now labelled by their actual severity: real breaking changes keep the red treatment, and housekeeping notices read as compatibility notices, in the panel and in the copied report text alike.' },
    ],
  },
  {
    version: '2.1.0-rc.13',
    date: '2026-09-01',
    highlights: 'A housekeeping release: the loose ends of rc.12, closed. The remote-session header stops re-playing its loading shimmer, never overstates persistence, and the browser button says which page is waiting for you.',
    changes: [
      { type: 'fix', description: 'The account shimmer plays once, not on every visit. When a remote session\'s account genuinely never arrives, the header\'s loading shimmer bows out after twenty seconds — but switching away and back re-mounted it with a fresh clock, so a chronically silent session greeted every return with another twenty seconds of shimmering. The header now remembers per session that the wait already ran its course and stays quiet on a revisit; the moment an account does arrive, the real pill shows, exactly as before.' },
      { type: 'fix', description: 'The header never overstates persistence. When a host refuses the persistence wrapper (no tmux on the remote, say), the session downgrades to a plain connection — but the header could keep reading SSH-Persistent because the downgrade arrived without anything else it repaints for. The connection pill now repaints on that downgrade, so what the header promises about surviving a dropped link is always the truth.' },
      { type: 'improvement', description: 'The browser button\'s tooltip says which page is waiting. With the pane open on your own page and an agent-pushed page queued behind it, the tooltip stacked two bare addresses with nothing saying which was which. The queued one is now labelled "Waiting:".' },
      { type: 'improvement', description: 'The Feature Guide and Ask Conductor catch up with the browser push. Both now explain how Claude hands you a page (the pill on the Browser button; nothing loads until you click), and the known-issues list gains an entry for the stray characters a plain SSH connection can occasionally leave in Claude\'s input on connect — what they are, why tmux sessions never see them, and that deleting them is the whole fix.' },
    ],
  },
  {
    version: '2.1.0-rc.12',
    date: '2026-08-31',
    highlights: 'A remote session stops being a second-class one. Local and remote sessions now report their status the same way, over the same channel, so an SSH session shows its account and its full usage — including the per-model bars — exactly like a session on your own machine. That also fixes the one host the status line never worked on: a Windows remote. The config dialog answers "where does this run?" properly for the first time: Connection is a choice of three (Local, SSH, SSH Persistent), and a new Runtime section runs the session inside a Docker or Podman container without you maintaining a shell one-liner. And a plan on the canvas is reviewed as a plan: Approve or Submit Revisions, with approval held back while a question is still open. A remote session you left running on a host is no longer out of sight either: it waits in a Remote Resumable section at the foot of the Running tab, one click from the same Claude and the same conversation, while launching a config always starts something new. And a config can be marked Allow Multi Spawn, so the ones you genuinely want several copies of get a copy-count control and a Select mode that launches a batch, and everything else is protected from a stray second launch.',
    changes: [
      { type: 'feature', description: 'A remote session shows its account and its usage, like a local one. An SSH session now reports over its own connection back to the app rather than by painting escape codes through the terminal, and it gathers the same figures a local session does: the signed-in account on the remote, the 5-hour and weekly limits, and the per-model bars (Fable included). So the account line on the session card, the account pill in the session header and the multi-account strip along the bottom all fill in for remote sessions. The strip no longer has to borrow a local session\'s numbers, and signing in with /login on the remote moves the session to the right account on the next tick. Nothing to install or re-run: it applies to sessions you launch from now on.' },
      { type: 'fix', description: 'The status line works on a Windows remote host. Connecting to a Windows machine over SSH used to show no status line at all, and the last release reported that as an upstream limitation to wait out. It was ours. A Windows remote stacks up to three terminal layers between Claude and your pane, and at least one of them rewrites or swallows the codes the status line was travelling in — bytes that never arrive cannot be read. The status now travels as data over the session\'s own connection instead, which no terminal layer touches, so Windows, Linux and macOS remotes all behave identically. It needs the built-in tools switched on (they are, by default); without them a remote session falls back to the old path.' },
      { type: 'improvement', description: 'The remote host is named once instead of three times. The floating host pill docked at the bottom-left of the terminal is gone, and so is the small "remote" tag on the command-bar clusters — both added last release, and both saying again what the header\'s SSH pill already says. The "this PC" mark stays on the partner shell, where it is genuinely surprising that the pane runs somewhere else.' },
      { type: 'feature', description: 'Connection is three plain choices: Local, SSH, or SSH Persistent. Persistence used to be a "Detachable" checkbox tucked below the SSH fields, which read like an option rather than what it is — a different kind of connection, one that keeps running when the link drops. It is now the third card beside Local and SSH, described where you choose it. Your saved configs are untouched and need no editing: one that had Detachable set simply opens as SSH Persistent.' },
      { type: 'feature', description: 'Run the session inside a container, without writing the command yourself. A new Runtime section on an SSH config asks where the session lands once it has connected: on the host, or in a Docker container. Pick the engine (docker or podman), name the container, choose whether to step into a running one or start a stopped one, optionally give a directory inside it, and say if the engine needs sudo — the app composes and runs the command. Claude runs inside the container with the full status line, account and usage, the same as anywhere else. If you already do this with a hand-written "after connecting" line, the dialog spots it and offers a one-click Convert; it never rewrites your command behind your back, and arbitrary prep still has its home under Advanced.' },
      { type: 'fix', description: 'Ending a container session ends the right Claude. The end that runs when you close a remote session now reaches inside the container and stops that session\'s Claude, matched on the session\'s own marker — so a second session sharing the same container carries on untouched, and a container no longer accumulates abandoned processes.' },
      { type: 'improvement', description: 'A container session says so. A small teal container mark sits on the session card and in the session header, with the container name on hover. It stands in for the SSH mark rather than sitting beside it: a container session runs without the persistence wrapper anyway, so one mark answers "where does this run?" without a second one arguing with it.' },
      { type: 'improvement', description: 'A plan on the canvas is reviewed as a plan. The two buttons are Approve and Submit Revisions — there is no Reject on a plan, because a plan is meant to go round again. Approve is held back while the plan still has an open question, or while you have a note you have not sent, and the panel says which: "Approve is unavailable: 2 open questions — answer them in a note and submit revisions". So an approval never arrives carrying work the agent has not seen. History reads REVISIONS rather than REJECTED for a round you sent back.' },
      { type: 'improvement', description: 'Less apparatus around a review. A plan drops the X-Ray switch, Region and Sketch entirely — it is reviewed by reading it and pointing at a step, and a padlocked control whose whole content was an apology for being disabled is worse than no control. The hover readout beside the page is one quiet row instead of a three-line box, and a plan carries no such row at all: there is nothing on a plan for it to read out, so it only ever showed a permanent idle dash while pushing the note box out of line with every other mode.' },
      { type: 'feature', description: 'A remote session carries your account\'s tools, not just its name. When an SSH session is signed in as one of the accounts you also use on this machine, its header gains the same claude.ai and Claude Code pills a local session has, each with a refresh, and its right-click menu gains Open artifacts and the sign-in items. Those checks and actions run here on your own machine for that account, which is why a session running on another host can still offer them. Switching the account stays local-only: a remote session uses the login on its own host, and with no matching local account it simply shows the account name as before.' },
      { type: 'improvement', description: 'The account sits at the far left of every session\'s status line. It used to move about -- on the left for a single-account session, over on the right for a switchable one -- so the one identity you most want to check was never in the same place twice. It is now the first item on the left for every session, local or remote, switchable or not, and the Settings and onboarding previews were reordered to match.' },
      { type: 'improvement', description: 'One pill names an SSH connection. A remote session\'s header used to show plain "SSH: user@host" text with a separate persistence dot beside it; that is now a single pill reading SSH, or SSH-Persistent when the session survives a dropped link, with the user@host address on the pill itself. A container session\'s pill carries the teal container mark in place of the word, with the same address beside it.' },
      { type: 'fix', description: 'No more "persistence unavailable" warning on a session that never asked to persist. A standard SSH session and a container session both run without the persistence wrapper by design, so the notice a persistent session shows when the remote has no tmux was appearing where it made no sense. It now shows only when a session actually tried to stay alive across a dropped link.' },
      { type: 'fix', description: 'A fresh remote session shows its account and usage the moment it connects. They used to fill in only after Claude\'s first status tick, so a new SSH session could sit briefly blank or need a restart to show them; the app now primes one status delivery as the session is set up, so the account and every usage bar (Fable included) are there from the start.' },
      { type: 'feature', description: 'The Session Watchdog now watches remote sessions too. The opt-in watcher that waits out a rate limit and types the retry for you used to run for local Claude sessions only; it now covers a Claude session running over SSH as well, so an overnight remote session picks itself back up the same way. It is careful about a remote pane: it arms only once Claude is actually running (never during the login or a password prompt), and it types a retry only when Claude\'s own prompt is the live screen -- never into a shell, a pager, or an auth prompt if Claude has exited. Still off by default, still Claude sessions only.' },
      { type: 'feature', description: 'A remote session you left running is waiting for you, in one place. Closing an SSH Persistent session and choosing Leave running used to put it out of sight, and the only way back was to launch the config again and answer a dialog about it. Left-running sessions now collect in a Remote Resumable section docked at the foot of the Running tab: one dim card each, carrying the account, the host you left it on, how long ago, and a pill reading Resumable or Unreachable. Click a card and you are back in the same Claude, in the same conversation, with the work it was doing. Right-click gives you Remove, which ends the session on the host as well as forgetting the card, so a machine is never left quietly holding a Claude you have finished with. The cards outlive the app: close the Conductor, open it tomorrow, and what you left running is still listed.' },
      { type: 'improvement', description: 'Launching a config always starts a new session. Clicking a saved config used to sometimes open a dialog first, asking whether you meant to resume something left running on a host, so the most ordinary action in the app behaved differently depending on state you could not see from the row you clicked. Launch now means launch, every time. Resuming is its own visible gesture, from the Remote Resumable section, where the thing you are resuming is in front of you.' },
      { type: 'improvement', description: 'A left-running remote is checked gently, and corrects itself. While the section is on screen the app pings each host about every ninety seconds, which needs no login and no shell; it only signs in to ask the host what is genuinely still running when there is a reason to, such as opening the section, coming back to the window, clicking a card, or starting the app. A small dot pulses beside a card while that check is in flight, and a host that was unreachable and comes back is re-checked once on its own, so a card recovers without you prodding it. If the remote turns out to have gone, you are told plainly and offered Remove or Start new rather than left waiting on a reconnection that cannot happen.' },
      { type: 'feature', description: 'Allow Multi Spawn: several copies of one config, on purpose. A saved config runs one session at a time unless you say otherwise, so a stray second click can no longer quietly start another Claude on the same project. Tick Allow Multi Spawn in a config and its row grows a copy-count control instead of the plain play button: set the number, press it, and that many sessions start at once, with the number remembered for next time. Click a config that is already running and is not a Multi Spawn one and a small note says exactly that, and offers Enable Multi Spawn and launch, rather than looking broken.' },
      { type: 'feature', description: 'Launch several configs in one go. A Select button on Saved and on Quick Start turns the rows into tick boxes; choose what you want and a bar along the bottom counts your selection and launches the lot. A config that is already running and is not a Multi Spawn one wears a padlock instead of a tick box and, when you click it, says why and offers to enable it. Cancel puts everything back.' },
      { type: 'improvement', description: 'A one-time page after this update asks where Multi Spawn belongs. Because the setting is off by default, anyone already running several copies of one config would have been stopped by it, so the app first looks at what you actually have (sessions running now, and remote sessions you left running on a host) and turns Multi Spawn on for any config it finds more than one of, marking each one with how many copies it found. The page then lists your configs with a switch each so you can change any of it. Skipping is safe, because the automatic part has already happened either way, and a config you switch off stays off: nothing turns it back on later.' },
      { type: 'improvement', description: 'Three kinds of remote session, told apart at a glance. A plain SSH session wears a blue SSH mark, a persistent one a green SSH mark with a chain link, and a session running inside a container wears a teal container mark on its own. They read the same way everywhere they appear: saved configs, Quick Start, the running sessions list and the session header. Only the header spells the address out beside the mark; the lists stay quiet.' },
      { type: 'improvement', description: 'Tidier sidebar rows. The account dot sits at the far left of a saved config, so the identity a session will run under is the first thing on the line rather than the second. Configs belonging to no section or group sit under their own Ungrouped heading that folds away, instead of trailing off the end of the list unannounced. And the buttons that appear when you hover a row no longer let the marks underneath show faintly through them.' },
      { type: 'fix', description: 'Setup stops, and says so, when Claude Code is not installed. Every session the Conductor launches is a Claude Code process, so with no claude command on the machine there was nothing for setup to configure, and it went ahead anyway and left you with a workbench that could not open a single session. Setup now checks before it opens the terminal step and, when it comes up empty, stops on a page that says so: the install command to copy, which check it ran, and a Retry. Install it, press Retry, and setup carries on from there. If you installed it in a terminal that was already open, restart the Conductor so it picks up the new PATH.' },
      { type: 'fix', description: 'A canvas decision reaches the agent even when it is mid-answer. Approving a version, or sending it back, writes a line into the session so the agent knows to collect it, and while the agent was busy composing a reply that line could be swallowed by its own interface. An approval carrying no notes was the worst case: it left nothing behind to recover from. Decisions made during a reply are now held and delivered the moment the agent finishes the turn. This narrows the window rather than closing it, so if the agent seems not to have noticed a verdict, just say it in chat; the Feature Guide carries this under Known issues.' },
      { type: 'fix', description: 'The review panel\'s live button is the one that looks live. On a plan with an open question, Approve is out of reach and the only move is Submit Revisions, but all three buttons were drawn much the same, so it was easy to press the inert Submit and see nothing happen. A blocked button is now plainly drawn as blocked, the one move actually available is highlighted, and the reason a submit is unavailable is printed under it in words rather than left in a tooltip that a disabled button can never show.' },
      { type: 'fix', description: 'A plan always opens ready to read. Sketch and Region belong to the session rather than to the page, so drawing over a mockup and then being handed a plan could leave you still holding the pen, on a surface with no way to put it down: the page would not scroll and a step would not take a note. A plan now always opens in plain browsing, whatever the previous surface left switched on.' },
    ],
  },
  {
    version: '2.1.0-rc.11',
  ...
  },
  {
    ...
    changes: [
      { type: 'fix', description: 'A session that ran in a git worktree resumes correctly after the worktree is removed. Previously, when the folder a conversation ran in had been cleaned up, resuming opened a fresh, empty session — the real conversation was still on disk but unreachable, sometimes showing a generated name like "session-…". The conversation is now found and resumed in place.' },
      { type: 'improvement', description: 'Renaming a session carries the name onto the conversation on disk. The name now survives outside the app and identifies the conversation when you resume — the resume list shows the name you gave a session rather than inferring one, and clearing a name clears it there too.' },
    ],
  },
  {
    version: '2.1.0-rc.4',
