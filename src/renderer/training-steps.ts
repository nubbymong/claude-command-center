...
export const trainingSteps: TrainingStep[] = [
  ...
  },
  {
    ...
    section: 'admin',
    summary:
      'Every session writes a structured log of inputs, outputs, and events. Logs lets you scroll back through any past session — search, filter by type, and replay output exactly as it appeared. v1.5.10 rewrites the loader so big logs paginate in 500-line chunks and the filter updates incrementally.',
    highlights: [
      'Chunked load — 500 entries per page, "Load older" pulls more on demand (v1.5.10)',
      'Incremental filter — typing into the search box no longer re-renders the whole buffer (v1.5.10)',
      'xterm theme follows the app theme via semantic tokens (v1.5.10)',
      'Search with regex + case-sensitive toggles',
      'Grouped by date with per-session size; scoped to the current workspace',
    ],
    howToTrigger: [
      { label: 'Open', value: 'Click  📜  in the sidebar nav' },
      { label: 'Search', value: 'Header search input or  Ctrl+F' },
