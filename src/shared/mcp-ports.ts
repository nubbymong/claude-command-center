 ...
 * since v1.0 and is referenced in user ~/.claude/settings.json). Dev mode
 * (electron-vite, isPackagedApp() === false) binds 19433 so a dev session
 * can coexist with a running production install on the same machine without
 * EADDRINUSE.
 *
