...
Once connected, the Runtime section decides where the session actually lands. On the host is the default. In a Docker container makes the app build and run the container command for you: pick the engine (docker or podman), name the container, choose Exec into running or Start stopped, optionally give a working directory inside the container, and tick the box if the engine needs sudo (the sudo password is stored in your OS credential store, exactly like an SSH password). Claude then runs inside the container with the status line, account and usage working normally. Ending a container session stops that session's own Claude inside the container and leaves any other session sharing that container alone. If you already enter a container with a hand-written command, the dialog recognises it and offers a one-click Convert to the structured Runtime; it never converts anything on its own. The free-text After connecting, run field is still there, under Advanced, for arbitrary preparation that is not a container: it runs first, then the Runtime command.`,
  ...
  },
  {
    ...
    body: `The Session Watchdog is an opt-in helper that keeps a stuck session moving. When Claude hits a usage limit, it reads the limit banner, waits out the reset time, and types the retry itself, so an overnight session picks itself back up instead of losing the hours; it also backs off on API overload and safeguard errors, with a capped number of attempts. It is careful about WHEN it types: nothing is sent while a permission prompt or picker is open, or while your own unsubmitted draft is in the input box, because a stray line would select in the menu or corrupt your draft. In those cases it defers and re-checks, rather than sending. It is off by default; turn it on under Settings, General, Session Watchdog, where you can also change the retry message, the number of attempts, and the silence window that drives the sleep indicator.

It works for Claude sessions whether they run locally or over SSH, and only for Claude sessions (never a plain terminal, a Codex session, or the Ask Conductor pane). A remote session is handled with extra care, because its screen is drawn by the other machine: the watchdog arms only once Claude is actually running on the remote, never during the login or while a password prompt is waiting, and before it types a retry it checks that Claude's own prompt is the live screen. If Claude has exited to a shell, or a pager or another program is on screen, it holds off rather than typing into it. For a persistent (tmux) remote session it also turns that session's tmux status bar off, so the sleep indicator reflects Claude genuinely going quiet rather than a status-bar refresh. An armed session, local or remote, shows its state in the Services panel at the top of the window and as a small badge on the session.`,
  },
  {
    id: 'github',
  ...
  },
  {
    ...
    body: `Each Claude session can show a live status line: the account, kept at the far left, then the model, tokens against the context window, an API-equivalent cost estimate, lines changed, duration, and your 5-hour and weekly rate limits. Pick exactly which elements appear in Settings, Status Line tab; its font and size live in Settings, Font & Size, under Regions; the master switch there turns the whole thing off for new sessions. It applies only to sessions launched from the Conductor and never changes your global Claude config. Local and remote sessions use the same delivery channel, so an SSH session shows the same elements a local one does, account and usage bars included, and a freshly connected remote session fills them in immediately rather than after its first tick; a remote session sends its status back through the connection the built-in tools already open, which is why leaving those on (the default) keeps remote status lines working everywhere.`,
  },
  {
    id: 'tools',
