...
Mouse wheel scrolls by arrow-key inside a Detachable SSH session. Classic terminal copy/paste (Settings, General; on by default) turns off Claude's mouse handling so you can select text with the mouse -- this now applies to SSH sessions too, matching local ones. On a Detachable SSH session Claude runs inside tmux, which does not have mouse mode on, so the wheel is delivered to Claude as arrow-key presses instead of scrolling history. Workarounds: turn Detachable off for that SSH config (Claude then runs without tmux and the wheel scrolls the terminal's own scrollback), or turn Classic terminal copy/paste off for the session (Claude takes the mouse back and scrolls its own view, at the cost of mouse text selection).`,
  },
]
