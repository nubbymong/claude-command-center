...
No status line on a WINDOWS remote host. Connecting over SSH to a Linux or macOS machine shows the live status line normally, but a session whose remote end is Windows currently shows none. This is an upstream Claude Code limitation on Windows hosts (its status line is not emitted in that environment), not something the Conductor drops: the connection, terminal and session all work — only the status line is missing. There is no workaround yet; it lights up automatically once the upstream fix ships.
