...
export const APP_KNOWLEDGE_SECTIONS: AppKnowledgeSection[] = [
  ...
  },
  {
    ...
    body: `Terminal text vanishing while other sessions are open, on 2.1.0-beta.15 and earlier. If a terminal kept its background colours but lost its characters, and resizing the window, scrolling it, or clicking into it brought the text straight back, that was GPU rendering. Drawing terminals on the GPU uses one shared store of character images for the whole app rather than one per terminal, so whenever any single session refreshed that store, every other open terminal lost its characters until something made it redraw. It needed two or more sessions open, which is why it looked random, and it got worse the more sessions were open. This is fixed from 2.1.0-beta.16: the sessions now tell each other when the shared store is rebuilt and the others repaint themselves on the next frame, so a rebuild no longer strands anybody. GPU rendering stays on by default. Anyone who turned it off themselves while working around the fault still has it off, because the fix did not rewrite saved settings; they can turn it back on in Settings, General, under Terminal. Either way the choice applies to terminals opened after the change, so existing sessions need reopening.`,
  },
]
