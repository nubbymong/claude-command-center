...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$sw=[System.IO.StreamWriter]::new('%CCC_LOGFILE%',$true);$sw.AutoFlush=$true;try{ npm run dev 2>&1 | ForEach-Object { Write-Host $_; $sw.WriteLine([string]$_) } } finally { $ErrorActionPreference='SilentlyContinue'; Write-Host '[ccc] cleaning up dev processes...'; $sw.WriteLine('[ccc] cleaning up dev processes...'); Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'chrome.exe' -and $_.CommandLine -match 'chrome-debug-9322' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }; foreach ($p in 5173,9847,19433,9322,19434) { Get-NetTCPConnection -LocalPort $p -State Listen | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force } }; Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'electron.exe' -and $_.ExecutablePath -match 'claude-command-center' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }; $sw.WriteLine('[ccc] cleanup done.'); $sw.Close() }"
