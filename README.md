...
- **Partner terminal and scratchpad** — every session carries a partner shell in the same tab, and a per-session Excalidraw whiteboard that persists with the config; freeze the webview to annotate a snapshot, or export the drawing straight into Claude.
