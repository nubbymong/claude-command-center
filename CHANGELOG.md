...
> The terminal corruption is understood, and the setting that causes it is now off by default. GPU rendering shared one cache of character images across every open session, so one session refreshing it wiped the text out of all the others — which is why it looked random, why it got worse the more sessions you had, and why resizing the window brought it back. Leave GPU rendering off and you will not see it.
