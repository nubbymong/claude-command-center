...
### Added
- The Usage page loads one account at a time, in place. Opening it shows a row per account straight away and fills each one in as its figure arrives, so a slow account no longer holds the whole page blank, and an account whose fetch fails says so on its own row, with a retry, instead of stalling the rest.
