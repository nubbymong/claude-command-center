...
- Ctrl+wheel zooms the canvas pane like a browser — over the chrome, the render or the notes panel. The level shows in the header and sticks for the session.
...
- The Saved Configs panel grew into a real launcher: cards or a compact list (chosen in Settings), find-as-you-type with auto-complete, categories, running sessions excluded from the launch list, and a launch-all for a category.
...
- Windows: tools built for the GUI subsystem (the Slic3r family, Inkscape, some Qt and Git components) could attach to the app's console and bleed their log text over your terminal. They are spawned without a console now, so their output goes nowhere near your session.
