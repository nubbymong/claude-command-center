...
> Terminals no longer lose their text when several sessions are open. GPU rendering shared one cache of character images across every open session, so one session refreshing that cache wiped the characters out of all the others — which is why it looked random, why it got worse the more sessions you had, and why resizing the window brought the text back. The sessions now tell each other when the cache is rebuilt and repaint themselves on the next frame, so GPU rendering stays on and the fault is gone.
...
### Changed
- GPU rendering stays ON by default, because the fault behind it is fixed rather than hidden. When any terminal rebuilds the shared cache of character images it now tells the others, and they repaint themselves on the next frame instead of sitting blank until you scrolled, resized or switched to them. Separately, a terminal whose graphics context keeps dying now stops trying to rebuild it after a few attempts and settles on the plain renderer — a flapping graphics driver used to send it into a rebuild loop that flickered and could take the display down with it, and that retry budget now resets once the context has been stable for a while, so an unrelated hiccup weeks later does not count against it. If you turned GPU rendering off yourself while working around the blank-text fault, it has been left off: the default changed, your saved settings did not. The checkbox is in Settings, General, under Terminal.
