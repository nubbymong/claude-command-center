...
- Release-line labels: apply `release-2.1` to any issue/PR on the current 2.1 line — every `in-beta` issue should also carry it so "what ships in 2.1?" stays accurate; apply `release-2.2` to work deferred past 2.1. Once a line’s x.y.0 has shipped, patch work carries its own `release-x.y.z` label (`release-2.1.1`) instead of the line label. **Invariant: `in-beta` (or `in-release`) + `release-2.2` on the same issue is a contradiction** — either lifecycle label means the fix already ships in 2.1, so it can't also be deferred. See CONTRIBUTING.md ("Release-line labels")
