...
- Release-line labels: `release-2.1` = targets the current 2.1 line (implied by a fix merged to `beta` or a PR based on it); `release-2.2` = deferred past 2.1. **Invariant: `in-beta` + `release-2.2` on the same issue is a contradiction** — `in-beta` means already merged to `beta` (ships in 2.1), so it can't also be deferred. See CONTRIBUTING.md ("Release-line labels")
