...
- Release-line labels: apply `release-2.1` to any issue/PR on the current 2.1 line — every `in-beta` issue should also carry it so "what ships in 2.1?" stays accurate; apply `release-2.2` to work deferred past 2.1. **Invariant: `in-beta` + `release-2.2` on the same issue is a contradiction** — `in-beta` means already merged to `beta` (ships in 2.1), so it can't also be deferred. See CONTRIBUTING.md ("Release-line labels")
